...
public class ShowDetailInfor extends AppCompatActivity {
    ...
    com.example.jlu.myapplication.CircleProgressBar cpb;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        adjust=intent.getFloatExtra("adjust",0);
        ...
        float lev=Float.valueOf(split[split.length-1]);
        lev+=adjust;
        int level = Math.round(lev);//返回的烟雾污染等级的值
        if(level<0)level=0;
        if(level>5)level=5;
        ...
        arrString.add("4级");
        arrString.add("5级");
        cpb = (com.example.jlu.myapplication.CircleProgressBar)findViewById(R.id.cpb);
